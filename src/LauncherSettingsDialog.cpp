...
LauncherSettingsDialog::LauncherSettingsDialog ( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxDialog ( parent, id, title, pos, size, style )
{
    ...
    // Connect Events
    m_listData->Connect ( wxEVT_COMMAND_LIST_ITEM_DESELECTED, wxListEventHandler ( LauncherSettingsDialog::OnItemDeselected ), NULL, this );
...
{
    // Disconnect Events
    m_listData->Disconnect ( wxEVT_COMMAND_LIST_ITEM_DESELECTED, wxListEventHandler ( LauncherSettingsDialog::OnItemDeselected ), NULL, this );
    m_listData->Disconnect ( wxEVT_COMMAND_LIST_ITEM_SELECTED, wxListEventHandler ( LauncherSettingsDialog::OnItemSelected ), NULL, this );
...
void LauncherSettingsDialog::OnEditClick ( wxCommandEvent& event )
{
    ...
    row_info.m_itemId = m_listData->GetNextItem ( -1,
                        wxLIST_NEXT_ALL,
...
void LauncherSettingsDialog::OnRemoveClick ( wxCommandEvent& event )
{
    if ( wxMessageBox ( _ ( "Do you really want to remove this launcher action?" ), _ ( "Really remove?" ), wxYES_NO ) == wxYES )
    {
        ...
        for ( ;; )
        {
            itemIndex = m_listData->GetNextItem ( itemIndex,
                                                  wxLIST_NEXT_ALL,
                                                  wxLIST_STATE_SELECTED );
...
wxArrayString LauncherSettingsDialog::GetLabels()
{
    ...
    {
        row_info.m_itemId = m_listData->GetNextItem ( i - 1,
                            wxLIST_NEXT_ALL );
        row_info.m_mask = wxLIST_MASK_TEXT;
        row_info.m_col = 0;
        m_listData->GetItem ( row_info );
...
LauncherItemDialog::LauncherItemDialog ( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxDialog ( parent, id, title, pos, size, style )
{
    this->SetSizeHints ( wxDefaultSize, wxDefaultSize );
...
wxString LauncherItemDialog::GetLabel()
{
    return m_tLabel->GetValue();
}

wxString LauncherItemDialog::GetCommand()
{
    return m_tCommand->GetValue();
}
