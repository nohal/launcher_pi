...
LauncherSettingsDialog::LauncherSettingsDialog(wxWindow* parent, wxWindowID id,
    ...
    : wxDialog(parent, id, title, pos, size, style)
{
    ...
    m_listData = new wxListCtrl(this, wxID_ANY, wxDefaultPosition,
        wxDefaultSize, wxLC_REPORT | wxLC_SINGLE_SEL);
    bSizerList->Add(m_listData, 1, wxALL | wxEXPAND, 5);

    m_cbCloseOnButton = new wxCheckBox(this, wxID_ANY,
        _("Close window on action"), wxDefaultPosition, wxDefaultSize, 0);
    ...
    // Connect Events
    m_listData->Connect(wxEVT_COMMAND_LIST_ITEM_DESELECTED,
        wxListEventHandler(LauncherSettingsDialog::OnItemDeselected), NULL,
        this);
    m_listData->Connect(wxEVT_COMMAND_LIST_ITEM_SELECTED,
        wxListEventHandler(LauncherSettingsDialog::OnItemSelected), NULL, this);
    m_bAdd->Connect(wxEVT_COMMAND_BUTTON_CLICKED,
        wxCommandEventHandler(LauncherSettingsDialog::OnAddClick), NULL, this);
    m_bEdit->Connect(wxEVT_COMMAND_BUTTON_CLICKED,
        wxCommandEventHandler(LauncherSettingsDialog::OnEditClick), NULL, this);
    m_bRemove->Connect(wxEVT_COMMAND_BUTTON_CLICKED,
        wxCommandEventHandler(LauncherSettingsDialog::OnRemoveClick), NULL,
        this);
...
{
    // Disconnect Events
    m_listData->Disconnect(wxEVT_COMMAND_LIST_ITEM_DESELECTED,
        wxListEventHandler(LauncherSettingsDialog::OnItemDeselected), NULL,
        this);
    m_listData->Disconnect(wxEVT_COMMAND_LIST_ITEM_SELECTED,
        wxListEventHandler(LauncherSettingsDialog::OnItemSelected), NULL, this);
    m_bAdd->Disconnect(wxEVT_COMMAND_BUTTON_CLICKED,
        wxCommandEventHandler(LauncherSettingsDialog::OnAddClick), NULL, this);
    m_bEdit->Disconnect(wxEVT_COMMAND_BUTTON_CLICKED,
        wxCommandEventHandler(LauncherSettingsDialog::OnEditClick), NULL, this);
    m_bRemove->Disconnect(wxEVT_COMMAND_BUTTON_CLICKED,
        wxCommandEventHandler(LauncherSettingsDialog::OnRemoveClick), NULL,
        this);
...
void LauncherSettingsDialog::OnAddClick(wxCommandEvent& event)
{
    ...
    if (m_itemdlg->ShowModal() == wxID_OK) {
        long itemIndex = m_listData->InsertItem(
            m_listData->GetItemCount(), m_itemdlg->GetLabel());
...
void LauncherSettingsDialog::OnEditClick(wxCommandEvent& event)
{
    ...
    row_info.m_itemId
        = m_listData->GetNextItem(-1, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED);
...
{
    if (wxMessageBox(_("Do you really want to remove this launcher action?"),
            _("Really remove?"), wxYES_NO)
        == wxYES) {
        long itemIndex = -1;
        for (;;) {
            itemIndex = m_listData->GetNextItem(
...
void LauncherSettingsDialog::SetItems(
    const wxArrayString& labels, const wxArrayString& commands, const bool hide)
{
...
LauncherItemDialog::LauncherItemDialog(wxWindow* parent, wxWindowID id,
    const wxString& title, const wxPoint& pos, const wxSize& size, long style)
    : wxDialog(parent, id, title, pos, size, style)
{
    this->SetSizeHints(wxDefaultSize, wxDefaultSize);
...
void LauncherItemDialog::SetValues(
    const wxString& label, const wxString& command)
{
